...
wikibase_name = env.get('WIKIBASE_NAME', 'wikibase')
wikibase_scheme = env.get('WIKIBASE_SCHEME', 'http')
wikibase_host = env.get('WIKIBASE_HOST', 'wikibase')
wikibase_ext_url = env.get('WIKIBASE_EXT_URL', 'http://www.localhost:81')
wdqs_scheme = env.get('WDQS_SCHEME', 'http')
wdqs_host = env.get('WDQS_FRONT_HOST', 'wdqs-front')
recon_name = env.get('RECON_NAME', 'recon service')
recon_scheme = env.get('RECON_SCHEME', 'http')
recon_host = env.get('RECON_HOST', 'reconcile')
external_recon_url = env.get('EXTERNAL_RECON_URL')
redis_scheme = env.get('REDIS_SCHEME', 'redis')
redis_host = env.get('REDIS_HOST', 'redis')

wikibase_url = wikibase_scheme + '://' + wikibase_host
wdqs_url = wdqs_scheme + '://' + wdqs_host + ':8834'
recon_url = recon_scheme + '://' + recon_host + ':8000'

# Endpoint of the MediaWiki API of the Wikibase instance
mediawiki_api_endpoint = wikibase_url + '/w/api.php'

# SPARQL endpoint
wikibase_sparql_endpoint = wdqs_url
...
import re
q_re = re.compile(r'(<?http?://' + re.escape(wikibase_ext_url) + r'/(entity|wiki)/)?(Q[0-9]+)>?')
q_re_group_id = 3
p_re = re.compile(r'(<?http?://' + re.escape(wikibase_ext_url) + r'/(entity/|wiki/Property:))?(P[0-9]+)>?')
p_re_group_id = 3

# Identifier space and schema space exposed to OpenRefine.
# This should match the IRI prefixes used in RDF serialization. For instance: http://www.localhost:81/wiki/Special:EntityData/Q10.rdf 
...
identifier_space = wikibase_ext_url + '/entity/'
schema_space = wikibase_ext_url + '/prop/direct/'
...
qid_url_pattern = wikibase_ext_url + '/wiki/Item:{{id}}'
...
redis_uri = redis_scheme + '://' + redis_host + ':6379/0?encoding=utf-8'
...
fallback_image_alt = 'KBGAWikibase-logo'
...
# http://www.localhost:81/wiki/Property:P1
...
wdt_prefix = 'kbwdt:'
#PREFIX kbwdt: <http://www.localhost:81/prop/direct/>
#PREFIX kbwd: <http://www.localhost:81/entity/>
#PREFIX wdt: <http://www.wikidata.org/prop/direct/>
#PREFIX wd: <http://www.wikidata.org/entity/>

# Sparql query used to fetch all the subclasses of a given item.
# http://www.localhost:81/wiki/Property:P2
# The '$qid' string will be replaced by the qid whose children should be fetched.

sparql_query_to_fetch_subclasses = f"""
PREFIX kbwdt: <{wikibase_url}/prop/direct/>
PREFIX kbwd: <{wikibase_url}/entity/>
...
# http://www.localhost:8834/#SELECT%20%3Fpid%20WHERE%20%7B%20%3Fpid%20wikibase%3ApropertyType%20wikibase%3AExternalId%20%7D
